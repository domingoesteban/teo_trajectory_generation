...
for ii=2:size(footprints_poses,2)-1,
    ...
    DSpose1 = pose_tr2rpy(pose_rpy2tr(footprints_poses(:,ii))\pose_rpy2tr(DSglobal_pose1));
    DSpose2 = pose_tr2rpy(pose_rpy2tr(footprints_poses(:,ii))\pose_rpy2tr(DSglobal_pose2));
    ...
    [cog_traj_step, dcog_traj_step, ddcog_traj_step] = com_traj_one_step(lambdaI, lambdaF, betaI, betaF, DSpose1, DSpose2, footprints_poses(:,ii), zc, g, Ts, t0, TDS1, TSS, TDS2);  
    
    % Convert foot reference to global reference
    cog_traj_step.data = transform_frame(pose_rpy2tr(footprints_poses(:,ii)), cog_traj_step.data);
%     dcog_traj_step.data = transform_frame(pose_rpy2tr(footprints_poses(:,ii)), dcog_traj_step.data);
%     ddcog_traj_step.data = transform_frame(pose_rpy2tr(footprints_poses(:,ii)), ddcog_traj_step.data);
