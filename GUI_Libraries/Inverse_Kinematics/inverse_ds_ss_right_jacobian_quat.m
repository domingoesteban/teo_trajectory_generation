function [q, dq, ddq] = inverse_ds_ss_right_jacobian_quat(q0, trajectory, h)
...
%     TODO2: It doesn't consider arms and CoM movement yet. PROBABLY
%     CODE IS IN THE END.
...
for jj=1:L-1
    ...
    if (trajectory.SF(jj)== -1) % left foot floating
...
%     [e_p_RH(:,jj) e_o_RH(:,jj)] = determine_error (trajectory.RH(:,jj),...
%         (pose_SF_RF+pose_quat2rpy(h.RF_T_CoM(q(:,jj))))+pose_quat2rpy(h.CoM_T_RH(q(:,jj))));
%     [e_p_LH(:,jj) e_o_LH(:,jj)] = determine_error (trajectory.LH(:,jj),...
%         (pose_SF_RF+pose_quat2rpy(h.RF_T_CoM(q(:,jj))))+pose_quat2rpy(h.CoM_T_LH(q(:,jj))));
%     
%     % Control signals for arms
%     u_RH = [Kp*e_p_RH(:,jj); Ko*e_o_RH(:,jj)];
%     u_LH = [Kp*e_p_LH(:,jj); Ko*e_o_LH(:,jj)];
