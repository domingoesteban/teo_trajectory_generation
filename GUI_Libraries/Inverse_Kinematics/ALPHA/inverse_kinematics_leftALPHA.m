function [q, q_dot, p, R, error_p, error_o] = inverse_kinematics_leftALPHA (Ts, T, q0, Kp, Ko,  xd, xd_dot, Rd, wd)
%initialization
iterations = T/Ts;
...
for kk = 1:iterations-1
    ...
    %present position
    R(:,:,kk) = evaluate_orientation_left (q(:,kk));
    p(:,kk) = evaluate_position_left (q(:,kk));
    %present quaternion
    ...
    %present velocity
    q_dot(:,kk) = evaluate_jacobian_left(q(:,kk))\[xd_dot(:,kk) + Kp*error_p(:,kk); wd(:,kk) + Ko*error_o(:,kk)];
    %next q
    q(:,kk+1) = q(:,kk) + q_dot(:,kk)*Ts;
end
R(:,:,iterations) = evaluate_orientation_left (q(:,iterations));
p(:,iterations) = evaluate_position_left (q(:,iterations));
