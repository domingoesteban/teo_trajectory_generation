...
% Last Modified by GUIDE v2.5 26-Nov-2013 20:44:30
...
% varargin   command line arguments to trajectory_generation (see VARARGIN)
clear trajectory d_trajectory dd_trajectory q dq ddq
global SETTINGS_TEO trajectory_points q0 input_type
global stretch_trajectory stretch_number humanoid_part total_points humanoid_part_to_plot
...
graphstab = uitabpanel(...
  ...
  'Units','normalized',...
  'Position',[0,0,1,1],...
  'FrameBackgroundColor',[0.0,0.4,1.0],...
  ...
  'FontWeight','bold',...
  'TitleBackgroundColor',[0.0,0.4,1.0],...
  'TitleForegroundColor',[1,1,1],...
  'PanelBackgroundColor',[0.078,0.169,0.549],...
...
set([handles.data_panel handles.save_operational_button handles.gen_joint_button handles.operational_panel],'Visible','off');
...
[q, dq, ddq] = ik_TEOPRUEBA(q0, trajectory, d_trajectory, dd_trajectory, SETTINGS_TEO.h, SETTINGS_TEO.parameters, SETTINGS_TEO, humanoid_part);
...
global SETTINGS_TEO humanoid_part_to_plot

set(handles.label_plots,'String','Plotting Operational Space Trajectory. Please wait...');
set(handles.label_plots,'BackGroundColor',[1 1 0.2]);
...
for jj = 1:6
    ...
    plot(handles.(strcat('operational_axes',num2str(jj))), time, trajectory.(humanoid_part_to_plot)(jj,:));hold on;
    ...
    plot(handles.(strcat('operational_axes',num2str(jj+6))), time, d_trajectory.(humanoid_part_to_plot)(jj,:));hold on;
    ...
    plot(handles.(strcat('operational_axes',num2str(jj+12))), time, dd_trajectory.(humanoid_part_to_plot)(jj,:));hold on;
...
set(handles.label_plots,'String','Plotting Operational Space Trajectory Complete!');
set(handles.label_plots,'BackGroundColor',[0.702 0.78 1]);
guidata(hObject,handles)
...
set(handles.label_plots,'String','Plotting Joint Space Trajectory. Please wait...');
set(handles.label_plots,'BackGroundColor',[1 1 0.2]);
...
[m,n]=size(qq);
for jj=1:m
    if strcmp(humanoid_part_to_plot,'CoM')
...
if strcmp(humanoid_part_to_plot,'CoM')
%clear other plots if humanoid_part_to_plot is CoM
    for kk=3:6,
...
set(handles.label_plots,'String','Plotting Joint Space Trajectory Complete!');
set(handles.label_plots,'BackGroundColor',[0.702 0.78 1]);
guidata(hObject,handles)
...
global humanoid_part input_type
update_points(humanoid_part, handles);
update_support_foot(handles);
stretch_number = get(hObject,'Value');
messages_master(handles)
set(handles.humanoid_part_panel, 'Title', ['Poses ' input_type ' - "Stretch ' stretch_trajectory{stretch_number} '"']);
show_selected_values(handles)
...
function joint_visualization_panel_SelectionChangeFcn(hObject, eventdata, handles)
global humanoid_part trajectory d_trajectory dd_trajectory humanoid_part_to_plot q dq ddq
humanoid_part_selected = get(eventdata.NewValue,'String');
set(handles.panel3, 'Title', humanoid_part_selected);

switch humanoid_part_selected
    case 'Right Arm'
        humanoid_part_to_plot = 'RH';
        
    case 'Left Arm'
        humanoid_part_to_plot = 'LH'; 
        
    case 'Right Leg'
        humanoid_part_to_plot = 'RF';
        
    case 'Left Leg'
        humanoid_part_to_plot = 'LF';
        
    case 'Torso'
        humanoid_part_to_plot = 'CoM';
end
...
for pp=1:total_points
    for ff=1:(length(SETTINGS_TEO.humanoid_fields)-1)
    end
% set (handles.T_val,'String',num2str(trajectory_points.T_val.(humanoid_part)(stretch_number)))
end
...
% --- Executes on selection change in option_popup.
function option_popup_Callback(hObject, eventdata, handles)
% hObject    handle to option_popup (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns option_popup contents as cell array
%        contents{get(hObject,'Value')} returns selected item from option_popup
global input_type
contents = cellstr(get(hObject,'String'));
input_type = contents{get(hObject,'Value')};
stretch_trajectory_popup_Callback(handles.stretch_trajectory_popup, eventdata, handles);


% --- Executes during object creation, after setting all properties.
function option_popup_CreateFcn(hObject, eventdata, handles)
% hObject    handle to option_popup (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
