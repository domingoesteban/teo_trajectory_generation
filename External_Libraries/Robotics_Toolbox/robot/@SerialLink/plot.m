...
function h = create_new_robot(robot, opt)
    ...
    if strcmp(robot.name, 'TEO_T')
        h.end_link = line(robot.lineopt{:});
        h.clavicle_right = line(robot.lineopt{:});
        h.clavicle_left = line(robot.lineopt{:});
        h.chest = line(robot.lineopt{:});
        
        [xCoM, yCoM, zCoM] = sphere; 
        h.CoM = surf(xCoM,yCoM,zCoM);
    end
    
    if strcmp(robot.name, 'TEO_LR')
        h.waist = line(robot.lineopt{:});
        h.hip =  line(robot.lineopt{:});
        h.foot = patch(zeros(4,1),zeros(4,1),zeros(4,1),'FaceColor', 'k', 'FaceAlpha', 0.5);
    end
    ...
    if opt.wrist,   
        ...
        h.z = line('xdata', [0;0], ...
            ...
            'zdata', [0;0], ...
            'color', 'blue');
        h.xt = text(0, 0, strcat(opt.wristlabel(1),'6'), 'FontWeight', 'bold', 'HorizontalAlignment', 'Center');
        h.yt = text(0, 0, strcat(opt.wristlabel(2),'6'), 'FontWeight', 'bold', 'HorizontalAlignment', 'Center');
        h.zt = text(0, 0, strcat(opt.wristlabel(3),'6'), 'FontWeight', 'bold', 'HorizontalAlignment', 'Center');

      % Ejes de las otras articulaciones  
        n = robot.n;
         for j=1:n,
            h.x_others(j)= line('xdata', [0;0], ...
                'ydata', [0;0], ...
                'zdata', [0;0], ...
                'color', 'red','LineWidth',2);
            h.y_others(j) = line('xdata', [0;0], ...
                'ydata', [0;0], ...
                'zdata', [0;0], ...
                'color', 'green','LineWidth',2);
            h.z_others(j) = line('xdata', [0;0], ...
                'ydata', [0;0], ...
                'zdata', [0;0], ...
                'color', 'blue','LineWidth',2);
            
            h.x_otherst(j) = text(0, 0, strcat('x',num2str(j-1)), 'FontWeight', 'bold', 'HorizontalAlignment', 'Center','color', 'red');
            h.y_otherst(j) = text(0, 0, strcat('y',num2str(j-1)), 'FontWeight', 'bold', 'HorizontalAlignment', 'Center','color', 'green');
            h.z_otherst(j) = text(0, 0, strcat('z',num2str(j-1)), 'FontWeight', 'bold', 'HorizontalAlignment', 'Center','color', 'blue');
         end;
